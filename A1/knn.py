...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train_vec = x_train[i].flatten()
        for j in range(num_test):
            test_vec = x_test[j].flatten()
            
            diff = train_vec - test_vec
            sq_diff = diff * diff
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      dists[i] = torch.sum((x_train[i] - x_test)**2, dim=1) # 广播机制
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_cross_test = torch.mm(x_train, x_test.t()) 
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, indices = torch.topk(dists, k=k, dim=0, largest=False)  # largest=False指取最小的k个距离
    
    for j in range(num_test):
        test_indice = indices[:,j]
        labels = y_train[test_indice]
        
        counts = torch.bincount(labels) # 统计每个标签的出现次数
        y_pred[j] = torch.argmax(counts) # 自带最小性
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_folds = list(x_train.chunk(num_folds)) # 默认 dim = 0
    ...
    for fold_idx in range(num_folds):
        # 训练集
        x_train_fold = torch.cat([x_train_folds[i] for i in range(num_folds) if i != fold_idx],dim=0)
        y_train_fold = torch.cat([y_train_folds[i] for i in range(num_folds) if i != fold_idx],dim=0)
        # 验证集
