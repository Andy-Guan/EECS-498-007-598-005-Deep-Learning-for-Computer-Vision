...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.ones(M,N) # 也可以 x = torch.full((M,N),3.14)
    x *= 3.14
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    first = ((start + 9) // 10) * 10  # 向上取整
    last = (stop // 10) * 10          # 向下取整
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    pos_sum = positive_vals.sum().item()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y = torch.cat([x_3d[0], x_3d[1]], dim=1) # 这道题我不会，AI用的是 cat 强行连接
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    min_indices = x.argmin(dim=1) # 找索引
