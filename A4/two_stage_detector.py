...
class RPNPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_channels in stem_channels:
            conv = nn.Conv2d(curr_channels, out_channels, kernel_size=3, stride=1, padding=1)
        ...
        # Replace "pass" statement with your code
        self.pred_obj = nn.Conv2d(curr_channels, num_anchors, kernel_size=1)
        self.pred_box = nn.Conv2d(curr_channels, num_anchors * 4, kernel_size=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            features = self.stem_rpn(feat)
            obj_logits = self.pred_obj(features)
            box_deltas = self.pred_box(features)

            B, _, H, W = obj_logits.shape
            obj_logits = obj_logits.permute(0, 2, 3, 1).contiguous().view(B, -1)
            box_deltas = box_deltas.view(B, self.num_anchors, 4, H, W).permute(0, 3, 4, 1, 2).contiguous().view(B, -1, 4)
...
def iou(boxes1: torch.Tensor, boxes2: torch.Tensor) -> torch.Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    tl = torch.max(boxes1[:, None, :2], boxes2[None, :, :2])  
...
def rcnn_get_deltas_from_anchors(
    ...
    bg_mask = (gt_boxes[:, 0] == -1) | (gt_boxes[:, 0] == -1e8)
    deltas[bg_mask] = -1e8
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class RPN(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            gt_targets_deltas = rcnn_get_deltas_from_anchors(
                anchor_boxes[sampled_idxs], matched_gt_boxes[sampled_idxs]
            )

            labels = torch.zeros_like(pred_obj_logits[sampled_idxs])
            labels[:len(fg_idx)] = 1.0  # 
            loss_obj = F.binary_cross_entropy_with_logits(
                pred_obj_logits[sampled_idxs], labels, reduction="none"
            )

            loss_box = torch.zeros_like(pred_boxreg_deltas[sampled_idxs])
            if len(fg_idx) > 0:
                loss_box[:len(fg_idx)] = F.l1_loss(
                    pred_boxreg_deltas[fg_idx], gt_targets_deltas[:len(fg_idx)], reduction="none"
                )
            ##################################################################
            #                         END OF YOUR CODE                       #
            ##################################################################
    ...
    def predict_proposals(
        ...
        for level_name in anchors_per_fpn_level.keys():
            ...
            for _batch_idx in range(level_obj_logits.shape[0]):
                ...
                num_pre_nms = min(self.pre_nms_topk, obj_logits.shape[0])
                topk_logits, topk_idx = torch.topk(obj_logits, num_pre_nms)
                topk_proposals = proposals[topk_idx]

                keep_idx = torchvision.ops.nms(topk_proposals, topk_logits, self.nms_thresh)
                keep_idx = keep_idx[:self.post_nms_topk]
                
                level_proposals_per_image.append(topk_proposals[keep_idx])
                ##############################################################
                #                        END OF YOUR CODE                    #
                ##############################################################
...
class FasterRCNN(nn.Module):
    ...
    def __init__(
        ...
        cls_pred = []
        # Replace "pass" statement with your code
        curr_channels = rpn.pred_net.stem_rpn[0].in_channels
        ...
        in_features = curr_channels * roi_size[0] * roi_size[1]
        
        linear = nn.Linear(in_features, num_classes + 1)
    ...
    def forward(
        ...
        gt_classes = matched_gt_boxes[sampled_idxs, 4].long()
        gt_classes = gt_classes + 1
        
        sampled_logits = pred_cls_logits[sampled_idxs]
        loss_cls = F.cross_entropy(sampled_logits, gt_classes)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return {
    ...
    def inference(
        ...
        fg_probs = probs[:, 1:]
        max_scores, max_classes = torch.max(fg_probs, dim=1)

        keep_mask = max_scores > test_score_thresh
        pred_scores = max_scores[keep_mask]
        pred_classes = max_classes[keep_mask]
