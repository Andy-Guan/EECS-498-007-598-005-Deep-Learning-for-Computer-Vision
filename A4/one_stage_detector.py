...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for out_dim in stem_channels:
            conv_cls = nn.Conv2d(current_in_channels, out_dim, kernel_size=3, stride=1, padding=1)
            torch.nn.init.normal_(conv_cls.weight, mean=0.0, std=0.01)
            torch.nn.init.constant_(conv_cls.bias, 0.0)
            stem_cls.append(conv_cls)
            stem_cls.append(nn.ReLU())
            
            conv_box = nn.Conv2d(current_in_channels, out_dim, kernel_size=3, stride=1, padding=1)
            torch.nn.init.normal_(conv_box.weight, mean=0.0, std=0.01)
            torch.nn.init.constant_(conv_box.bias, 0.0)
            stem_box.append(conv_box)
            stem_box.append(nn.ReLU())
            
            current_in_channels = out_dim
        ...
        # Replace "pass" statement with your code
        final_in_channels = stem_channels[-1]
        
        self.pred_cls = nn.Conv2d(final_in_channels, num_classes, kernel_size=3, stride=1, padding=1)
        self.pred_box = nn.Conv2d(final_in_channels, 4, kernel_size=3, stride=1, padding=1)
        self.pred_ctr = nn.Conv2d(final_in_channels, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, features in feats_per_fpn_level.items():
            ...
            cls_stem_out = self.stem_cls(features)
            cls_out = self.pred_cls(cls_stem_out) 
            
            box_stem_out = self.stem_box(features)
            box_out = self.pred_box(box_stem_out)
            ctr_out = self.pred_ctr(box_stem_out)
            

            B, _, H, W = features.shape
            class_logits[level_name] = cls_out.permute(0, 2, 3, 1).reshape(B, H * W, -1)
            boxreg_deltas[level_name] = box_out.permute(0, 2, 3, 1).reshape(B, H * W, -1)
            centerness_logits[level_name] = ctr_out.permute(0, 2, 3, 1).reshape(B, H * W, -1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    xc, yc = locations[:, 0], locations[:, 1]
    deltas = torch.clamp(deltas, min=0.0) # 这里 clip them to zero
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # 防止爆炸
    max_lr = torch.clamp(max_lr, min=1e-6)
    max_tb = torch.clamp(max_tb, min=1e-6)
    ratio_lr = torch.clamp(min_lr / max_lr, min=0.0)
    ratio_tb = torch.clamp(min_tb / max_tb, min=0.0)

    centerness = torch.sqrt(ratio_lr * ratio_tb)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        # 提取每一层特征图的 shape
        shape_per_fpn_level = {level: feat.shape for level, feat in fpn_feats.items()}
        ...
        is_foreground = (gt_classes >= 0)

        targets_cls = torch.zeros_like(pred_cls_logits)
        targets_cls[is_foreground] = F.one_hot(
            gt_classes[is_foreground].long(), 
            num_classes=self.num_classes
        ).to(pred_cls_logits.dtype)
        
        loss_cls = sigmoid_focal_loss(pred_cls_logits, targets_cls, reduction="none").sum(dim=2)
        loss_box = F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none").sum(dim=2)
        loss_box[~is_foreground] = 0.0
        flat_gt_deltas = matched_gt_deltas.view(-1, 4)
        centerness_targets = fcos_make_centerness_targets(flat_gt_deltas).view(pred_ctr_logits.shape)
        safe_centerness_targets = centerness_targets.clamp(min=0.0, max=1.0)
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, safe_centerness_targets, reduction="none").sum(dim=2)
