...
class Conv(object):
    ...
    @staticmethod
    def backward(dout, cache): # 这个反向传播实在太难了 
        """
        ...
        # 对 N (dim=0), H_out (dim=2), W_out (dim=3) 求和
        db = torch.sum(dout, dim=(0, 2, 3))

        # 初始化 dw 和带 Padding 的 dx_pad
        dw = torch.zeros_like(w)
        dx_pad = torch.zeros((N, C, H + 2 * pad, W + 2 * pad), dtype=x.dtype, device=x.device)
        ...
        for i in range(H_out):
            for j in range(W_out):
                ...
                # --- 计算 dw ---
                # (F, N) @ (N, C*HH*WW) -> (F, C*HH*WW)
                dw_flat_step = dout_step.t() @ patch_flat
                # 累加到全局的 dw 上
                dw += dw_flat_step.view_as(w) # 修改形状

                # --- 计算 dx ---
                # 误差向后传导给像素点：(N, F) @ (F, C*HH*WW) -> (N, C*HH*WW)
                dx_patch_flat = dout_step @ w_flat
                # 恢复成图像块的形状 (N, C, HH, WW)
                dx_patch = dx_patch_flat.view(N, C, HH, WW)
                # 累加到 dx_pad 对应的区域中
                dx_pad[:, :, h_start:h_end, w_start:w_end] += dx_patch
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        Hout = 1 + (x.shape[2] - pool_height) // stride
        Wout = 1 + (x.shape[3] - pool_width) // stride
        out = torch.zeros((x.shape[0], x.shape[1], Hout, Wout), dtype=x.dtype, device=x.device)
        for i in range(Hout):
            for j in range(Wout):
                h_start = i * stride
                ...
                out[:, :, i, j] = torch.max(x[:, :, h_start:h_end, w_start:w_end].reshape(x.shape[0], x.shape[1], -1), dim=2).values
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(Hout):
            for j in range(Wout):
                ...
                max_val = torch.amax(patch, dim=(2, 3), keepdim=True) # 无索引，可以保留张量形状
                mask = (patch == max_val)

                dout_step = dout[:, :, i, j].view(N, C, 1, 1)
                dx[:, :, h_start:h_end, w_start:w_end] += dout_step * mask
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        H_pool = H // 2 # 后面的loss决定的
        W_pool = W // 2
        flat_dim = num_filters * H_pool * W_pool
        
        self.params['W2'] = torch.randn(flat_dim, hidden_dim, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        # 切记用后面写好的，否则非常慢
        out_pool, cache_pool = Conv_ReLU_Pool.forward(X, W1, b1, conv_param,pool_param)

        # 将形状 (N, num_filters, H/2, W/2) 转换为 (N, D)
        N = X.shape[0]
        out_pool_flat = out_pool.view(N, -1)
        
        out_lin1, cache_lin1 = Linear_ReLU.forward(out_pool_flat, W2, b2)
        ...
        dout_lin1, dw3, db3 = Linear.backward(dscores, cache_lin2)
        grads['W3'] = dw3 + 2 * self.reg * W3
        grads['b3'] = db3
        
        dout_pool_flat, dw2, db2 = Linear_ReLU.backward(dout_lin1, cache_lin1)
        grads['W2'] = dw2 + 2 * self.reg * W2
        grads['b2'] = db2
        
        # 将展平的梯度恢复为池化层输出的三维空间形状
        dout_pool = dout_pool_flat.view(out_pool.shape)
        
        dx, dw1, db1 = Conv_ReLU_Pool.backward(dout_pool, cache_pool)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers - 1):
            ...
            if weight_scale == 'kaiming':
                self.params[f'W{idx}'] = kaiming_initializer(
                    current_C, num_F, K=filter_size, relu=True, device=device, dtype=dtype)
            else:
                self.params[f'W{idx}'] = torch.randn(
                    num_F, current_C, filter_size, filter_size, dtype=dtype, device=device
                ) * weight_scale
                
            self.params[f'b{idx}'] = torch.zeros(num_F, dtype=dtype, device=device)
            

            if self.batchnorm:
                self.params[f'gamma{idx}'] = torch.ones(num_F, dtype=dtype, device=device)
                self.params[f'beta{idx}'] = torch.zeros(num_F, dtype=dtype, device=device)
        ...
        flat_dim = current_C * current_H * current_W
    ...
    def loss(self, X, y=None):
        ...
        dout, dw_last, db_last = Linear.backward(dscores, caches.pop())
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x_centered, std, x_norm, gamma)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x_centered, std, x_norm, gamma = cache
        N, D = dout.shape
        ...
        dx_norm = dout * gamma
        dx = (1.0 / (N * std)) * (N * dx_norm - torch.sum(dx_norm, dim=0) - x_norm * torch.sum(dx_norm * x_norm, dim=0))
    ...
    def backward_alt(dout, cache):
        ...
        dgamma = torch.sum(dout * x_norm, dim=0)
        dbeta = torch.sum(dout, dim=0)
        dx = (gamma / (N * std)) * (N * dout - dbeta - x_norm * dgamma)
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # 将空间维度合并入批次维度: (N, C, H, W) -> (N, H, W, C) -> (N * H * W, C)
        # contiguous() 是为了保证内存连续
