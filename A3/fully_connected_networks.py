...
class FullyConnectedNet(object):
    ...
    where dropout is optional, and the {...} block is repeated L - 1 times. # 数量是不定的
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        #######################################################################
        # Replace "pass" statement with your code
        dims = [input_dim] + hidden_dims + [num_classes] # 维度拼接
    ...
    def loss(self, X, y=None):
        ...
        out = X
        caches = {}       
        dropout_caches = {}  # 存放 Dropout 层的缓存
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.use_dropout:
                ...
                dropout_caches[f'layer{i+1}'] = drop_cache
        ...
        caches[f'layer{self.num_layers}'] = final_cache
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(self.num_layers):
            W = self.params[f'W{i+1}']
            loss += 0.5 * self.reg * torch.sum(W ** 2) # 有 0.5 的正则化项
            
        dout, dw_last, db_last = Linear.backward(dscores, caches[f'layer{self.num_layers}'])
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_caches[f'layer{i}'])
...
def rmsprop(w, dw, config=None):
    ...
    config['cache'] = config['decay_rate'] * config['cache'] + (1 - config['decay_rate']) * (dw ** 2)
        ...
        # 2. 根据自适应学习率更新权重 (除以根号下的 cache 加上 epsilon)
