...
def svm_loss_naive(
    ...
    Inputs:
    - W: A PyTorch tensor of shape (D, C) containing weights. # 权重
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i]) # W 的转置 乘上这个样本的特征向量，得到这个样本对于每个类别的得分
    ...
    #############################################################################
    # Replace "pass" statement with your code   
    dW /= num_train  # 除以训练样本的数量，得到平均梯度
    dW += 2 * reg * W  # 正则化项的梯度是 2 * reg * W
...
def svm_loss_vectorized(
    ...
    # Replace "pass" statement with your code
    num_train = X.shape[0]
    scores = X.mm(W)  # 计算所有样本的得分矩阵
    correct_class_scores = scores[torch.arange(num_train), y].view(-1, 1)  # 获取正确类别的得分，并调整形状为 (N, 1)
    margins = torch.clamp(scores - correct_class_scores + 1, min=0)  # 计算 margin，并将负值置为 0
    margins[torch.arange(num_train), y] = 0  # 将正确类别的 margin 置为 0
    loss = margins.sum() / num_train  # 计算平均损失
    loss += reg * torch.sum(W * W)  # 添加正则化项
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    dW = X.t().mm(mask) / num_train  # 计算平均梯度
    dW += 2 * reg * W 
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    scores = X.mm(W)  
    _, y_pred = scores.max(dim=1)  # _表示具体分数，我们不需要知道
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    for i in range(2, 6):
        ...
        regularization_strengths.append(10 ** (i - 3))
...
def softmax_loss_vectorized(
    ...
    loss = (-correct_class_scores + torch.log(sum_exp_scores)).mean() + reg * torch.sum(W * W)
    p = exp_scores / sum_exp_scores 
    p[torch.arange(num_train), y] -= 1.0 
    dW = X.t().mm(p)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
