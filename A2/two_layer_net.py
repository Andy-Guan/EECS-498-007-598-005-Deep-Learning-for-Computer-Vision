...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    just for this time (you can use it from A3). # 还不让用 torch

    Inputs:
    - params: a dictionary of PyTorch Tensor that store the weights of a model. # 参数表保存权重
...
def nn_forward_backward(
    ...
    scale the losses/gradients by the batch size. # 除以批次大小
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores -= scores.max(dim=1, keepdim=True)[0] # 防止计算机出故障，不要最大值
    scores_exp = torch.exp(scores)
    scores_exp_sum = torch.sum(scores_exp, dim=1, keepdim=True)
    correct_class_scores_exp = scores_exp[torch.arange(N), y].unsqueeze(1)
    loss = -torch.sum(torch.log(correct_class_scores_exp / scores_exp_sum)) / N # 负号也可以放在里面
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    P = torch.zeros_like(scores_exp)
    P = scores_exp / scores_exp_sum
    P[torch.arange(N), y] -= 1.0
    dscores = P / N
...
def nn_predict(
    ...
    y_pred = torch.argmax(scores, dim=1) # 找到位置
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def find_best_net(
    ...
    learning_rates, hidden_sizes, regularizations, lr_decays = get_param_set_fn()
