...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        ...
        for j in range(K):
            dot_product = torch.dot(query[i], key[j]) # 转置
            scores[j] = dot_product / torch.sqrt(torch.tensor(M, dtype=torch.float))
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    out = torch.zeros(query.shape)
    for n in range(N):
        for i in range(K):
            q = query[n, i] 
            k_mat = key[n]  
            scores = torch.matmul(k_mat, q) 
            scores = scores / torch.sqrt(torch.tensor(M, dtype=torch.float))
            weights = F.softmax(scores, dim=0)
            v_mat = value[n]
            out[n, i] = torch.matmul(weights, v_mat)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    out = out / torch.sqrt(torch.tensor(M, dtype=torch.float))
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(out, dim=-1)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        self.k.weight.data.uniform_(-c_k, c_k)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.output_projection = nn.Linear(num_heads * dim_out, dim_in)
        c = (6 / (num_heads * dim_out + dim_in)) ** 0.5
        self.output_projection.weight.data.uniform_(-c, c)
    ...
    def forward(
        ...
        for attention in self.attention_heads:
            head_output = attention(query, key, value, mask)
            if y is None:
                y = head_output
            else:
                y = torch.cat((y, head_output), dim=-1)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        linear1.weight.data.uniform_(-c, c)
        ...
        linear2.weight.data.uniform_(-c, c)
        linear2.bias.data.zero_()

        self.feedforward = nn.Sequential(linear1, relu, linear2)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    mask = torch.triu(torch.ones((K, K), dtype=torch.bool, device=seq.device), diagonal=1).unsqueeze(0).expand(N, -1, -1) # 上三角矩阵
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    pos = torch.arange(K).unsqueeze(1)
    i = torch.arange(M).unsqueeze(0)
