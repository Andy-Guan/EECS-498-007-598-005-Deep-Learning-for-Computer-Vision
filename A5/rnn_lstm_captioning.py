...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = torch.nn.functional.cross_entropy(x.transpose(1, 2), y, ignore_index=(ignore_index if ignore_index is not None else -100), reduction='sum') / x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == 'attn':
            img_features = img_features.permute(0, 2, 3, 1) # (N, 4, 4, D)
            h0 = self.feature_proj(img_features).permute(0, 3, 1, 2) # 默认对最后一个维度线性，之后变回 (N, H, 4, 4)
        else:
            img_features = img_features.mean(dim=(2, 3)) # 平均池化
            h0 = self.feature_proj(img_features) # (N, H)

        word_embeds = self.word_embed(captions_in) # (N, T, W)

        hidden_states = self.rnn(word_embeds, h0) # (N, T, H)

        scores = self.vocab_proj(hidden_states) # (N, T, V)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == 'attn':
            img_features = img_features.permute(0, 2, 3, 1) # (N, 4, 4, D)
            A = self.feature_proj(img_features).permute(0, 3, 1, 2) # (N, H, 4, 4)
            h = A.mean(dim=(2, 3)) # (N, H)
            c = h.clone() # (N, H)
        else:
            img_features = img_features.mean(dim=(2, 3)) # (N, D)
            h = self.feature_proj(img_features) # (N, H)
        
            if self.cell_type == 'lstm':
                c = torch.zeros_like(h) # (N, H)

        # 初始化第一个输入词: <START> token (确保其设备与图像一致)
        current_word = images.new(N).fill_(self._start).long()  # (N,)

        for t in range(max_length):
            word_embed = self.word_embed(current_word) # (N, W)
            ...
            scores = self.vocab_proj(h) # (N, V)

            # 直接找最大值的索引
            current_word = torch.argmax(scores, dim=1) # (N,)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        a_i, a_f, a_o, a_g = a.chunk(4, dim=1) # 切块
        
        i = torch.sigmoid(a_i) # 输入门 Input gate
        f = torch.sigmoid(a_f) # 遗忘门 Forget gate
        o = torch.sigmoid(a_o) # 输出门 Output gate
        g = torch.tanh(a_g)    # 块输入 Block input
